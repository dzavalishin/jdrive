...
public class MiscGui {
	...
	static boolean _fios_path_changed;
	static boolean _savegame_sort_dirty;
	...
	static void LandInfoWndProc(Window w, WindowEvent e)
	{
		if (e.event == WindowEvents.WE_PAINT) {
			...
			DrawStringCentered(140, 27, Str.STR_01A7_OWNER, 0);
			...
			Global.SetDParam(0, TileX(lid.tile));
			Global.SetDParam(1, TileY(lid.tile));
			Global.SetDParam(2, Str.STR_SPEC_USERSTRING);
			DrawStringCentered(140, 49, Str.STR_LANDINFO_COORDS, 0);
			...
			DrawStringCentered(140,60, Str.STR_01A8_LOCAL_AUTHORITY, 0);

			{
				char buf[512];
				char *p = Global.GetString(buf, Str.STR_01CE_CARGO_ACCEPTED);
				boolean found = false;
				...
				if (found) Gfx.DrawStringMultiCenter(140, 76, BindCString(buf), 276);
			}

			if (lid.td.build_date != 0) {
				Global.SetDParam(0,lid.td.build_date);
				DrawStringCentered(140,71, Str.STR_BUILD_DATE, 0);
			}
		}
	}
	...
	static void Place_LandInfo(TileIndex tile)
	{
		...
		p = GetPlayer(Global._local_player < Global.MAX_PLAYERS ? Global._local_player : 0);

		old_money = p.money64;
		p.money64 = p.player_money = 0x7fffffff;
		lid.costclear = DoCommandByTile(tile, 0, 0, 0, Cmd.CMD_LANDSCAPE_CLEAR);
		p.money64 = old_money;
		...
		//#if defined(_DEBUG)
			...
			DEBUG(misc, 0) ("m1           = %#x", tile.getMap().m1);
			DEBUG(misc, 0) ("m2           = %#x", tile.getMap().m2);
	...
	void PlaceLandBlockInfo()
	{
		if (Hal._cursor.sprite == Sprite.SPR_CURSOR_QUERY) {
			...
			SetObjectToPlace(Sprite.SPR_CURSOR_QUERY, 1, 1, 0);
		}
	}
	...
	static void AboutWindowProc(Window w, WindowEvent e)
	{
		...
		case WindowEvents.WE_PAINT: {
			...
			DrawStringCentered(210, 17, Str.STR_00B6_ORIGINAL_COPYRIGHT, 0);
			DrawStringCentered(210, 17 + 10, Str.STR_00B7_VERSION, 0);
			...
			DrawStringCentered(210, w.height - 15, Str.STR_00BA_COPYRIGHT_OPENTTD, 0);
		}	break;
	...
	*/
	...
	{
		final Widget *wi = w.widget + wid;
		final Textbuf *tb = &WP(w,querystr_d).text;
	...
	static void QueryStringWndProc(Window w, WindowEvent e)
	{
		...
		case WindowEvents.WE_PAINT:
			Global.SetDParam(0, WP(w,querystr_d).caption);
			DrawWindowWidgets(w);
	...
	void ShowQueryString(StringID str, StringID caption, int maxlen, int maxwidth, WindowClass window_class, WindowNumber window_number)
	{
		...
		Global.GetString(_edit_str_buf, str);
	...
	static void SaveLoadDlgWndProc(Window w, WindowEvent e)
	{
		...
		case WindowEvents.WE_PAINT: {
			...
			Gfx.GfxFillRect(w.widget[7].left + 1, w.widget[7].top + 1, w.widget[7].right, w.widget[7].bottom, 0xD7);
			Gfx.DoDrawString(
	...
	void ShowSaveLoadDialog(int mode)
	{
		...
		SetObjectToPlace(Sprite.SPR_CURSOR_ZZZ, 0, 0, 0);
		Window.DeleteWindowById(Window.WC_QUERY_STRING, 0);
		Window.DeleteWindowById(Window.WC_SAVELOAD, 0);
	...
	static void SelectScenarioWndProc(Window  w, WindowEvent  e)
	{
		...
		case WindowEvents.WE_PAINT:
		{
			...
			DrawString(4, 32, Str.STR_4010_GENERATE_RANDOM_NEW_GAME, 9);
	...
	static final CheatEntry _cheats_ui[] = {
		{CE_CLICK, 0, Str.STR_CHEAT_MONEY, 					&_cheats.money.value, 					&_cheats.money.been_used, 					&ClickMoneyCheat,					0, 0, 0},
		{CE_UINT8, 0, Str.STR_CHEAT_CHANGE_PLAYER, 	&Global._local_player, 								&_cheats.switch_player.been_used,		&ClickChangePlayerCheat,	0, 11, 1},
		{CE_BOOL, 0, Str.STR_CHEAT_EXTRA_DYNAMITE,	&_cheats.magic_bulldozer.value,	&_cheats.magic_bulldozer.been_used, null,											0, 0, 0},
		{CE_BOOL, 0, Str.STR_CHEAT_CROSSINGTUNNELS,	&_cheats.crossing_tunnels.value,&_cheats.crossing_tunnels.been_used,null,											0, 0, 0},
		{CE_BOOL, 0, Str.STR_CHEAT_BUILD_IN_PAUSE,	&_cheats.build_in_pause.value,	&_cheats.build_in_pause.been_used,	null,											0, 0, 0},
		{CE_BOOL, 0, Str.STR_CHEAT_NO_JETCRASH,			&_cheats.no_jetcrash.value,			&_cheats.no_jetcrash.been_used,			null,											0, 0, 0},
		{CE_BOOL, 0, Str.STR_CHEAT_SETUP_PROD,			&_cheats.setup_prod.value,			&_cheats.setup_prod.been_used,			null,											0, 0, 0},
		{CE_UINT8, 0, Str.STR_CHEAT_SWITCH_CLIMATE, &GameOptions._opt.landscape, 								&_cheats.switch_climate.been_used,	&ClickChangeClimateCheat,-1, 4, 1},
		{CE_UINT8, 0, Str.STR_CHEAT_CHANGE_DATE,		&_cur_year,											&_cheats.change_date.been_used,			&ClickChangeDateCheat,	 -1, 1, 1},
	};
		...
		case WindowEvents.WE_PAINT: {
			...
			for (i = 0; i != lengthof(_cheats_ui); i++) {
				...
				if (ce.type == CE_BOOL) {
					...
					DrawStringCentered(x + 25, y + 1, Str.STR_6819, 0);
					DrawStringCentered(x + 35, y + 1, Str.STR_681A, 0);
