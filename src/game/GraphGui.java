...
public class GraphGui 
{
	...
	static void GraphLegendWndProc(Window w, WindowEvent e)
	{
		...
		case WE_PAINT:
			...
			Player.forEach( (p) ->
			{
				if (!p.is_active) _legend_excludebits = BitOps.RETSETBIT(_legend_excludebits, p.index.id);
			});
	...
	static void SetupGraphDrawerForPlayers(GraphDrawer gd)
	{
		...
		Player.forEach((p) ->
		{
			if (!p.is_active) excludebits[0] = BitOps.RETSETBIT(excludebits[0],p.index.id);
		});
		...
		Player.forEach((p) ->
		{
			if (p.is_active) nums[0] = Math.max(nums[0],p.num_valid_stat_ent);
		});
	...
	static void CompanyLeagueWndProc(Window w, WindowEvent e)
	{
		...
		case WE_PAINT: {
			...
			Player.forEach( (p) ->
			{
				if (p.is_active) plist[pl_num[0]++] = p;
			});
	...
	static void PerformanceRatingDetailWndProc(Window w, WindowEvent e)
	{
		...
		case WE_PAINT: {
			...
			for (i=0;i<Global.MAX_PLAYERS;i++) {
				if (!Player.GetPlayer(i).is_active) {
