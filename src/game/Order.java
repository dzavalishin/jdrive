...
public class Order implements Serializable 
{
	...
	static int CmdInsertOrder(int x, int y, int flags, int p1, int p2)
	{
		...
		case OT_GOTO_STATION: {
			...
			if (!st.IsValidStation() ||
					(st.airport_type != Airport.AT_OILRIG && !(st.IsBuoy()) && !Player.CheckOwnership(st.owner) && !MunicipalAirport.ownerHandler(st.owner)))
				return Cmd.CMD_ERROR;

			//MA checks
			if(Global._patches.allow_municipal_airports.get() && !MunicipalAirport.withinVehicleQuota(Station.GetStation(new_order.station))) {
	...
	static int CmdCloneOrder(int x, int y, int flags, int p1, int p2)
	{
		...
		//MA checks
		if(MunicipalAirport.vehicleServesMS(Vehicle.GetVehicle(veh_src)) > 0) 
		{
			for(int i =  1; i <= MunicipalAirport.vehicleServesMS(Vehicle.GetVehicle(veh_src)) ; i++) 
			{
				Station st = Station.GetStation(MunicipalAirport.find_MS_InVehicleOrders(Vehicle.GetVehicle(veh_src), i).id);
